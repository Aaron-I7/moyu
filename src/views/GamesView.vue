  ...
  <div class="page">
    <div class="page-inner">
      ...
      <div v-if="games.length > 0" class="module-list">
        ...
        >
          ...
          <div class="card-content">
            <h3>{{ resolveModuleName(t, game) }}</h3>
            <p>{{ resolveModuleDescription(t, game) }}</p>
          </div>
          <div v-if="resolveModuleTags(t, game).length > 0" class="card-tags">
            <span v-for="tag in resolveModuleTags(t, game).slice(0, 3)" :key="tag" class="tag">
              {{ tag }}
            </span>
          </div>
        </div>
      ...
      <div v-else class="empty">
        ...
        <h3>{{ t('views.emptyGame') }}</h3>
        <p>{{ t('views.emptyGameDesc') }}</p>
      </div>
    </div>
  </div>
</template>
